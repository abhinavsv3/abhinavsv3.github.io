...
title: 'vLLM vs TensorRT-LLM: Architecture Deep-Dive'
description: "Both solve efficient LLM serving, but with fundamentally different approaches. vLLM optimizes at runtime; TensorRT-LLM optimizes at compile time."
pubDate: '2025-12-22'
...
LLM inference has [two distinct phases](/blog/scaling-foundation-model-inference):
...
- [FlashAttention](/blog/flash-attention-in-practice) kernels selected dynamically
...
**NGC Pre-built Assets:** The [NVIDIA NGC Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/tensorrt-llm/containers/release) now offers pre-compiled engines for popular models (Llama, Mistral, etc.) on common GPU types. Download and run, no 40-minute compilation.

**AutoDeploy (Beta):** The biggest shift is [AutoDeploy](https://nvidia.github.io/TensorRT-LLM/torch/auto_deploy/auto-deploy.html), which takes off-the-shelf PyTorch models from HuggingFace and automatically compiles them to TensorRT-LLM without manual API rewrites. It currently supports 100+ text-to-text LLMs.
...
**Workload matters:** These numbers are [highly dependent on your use case](/blog/maximizing-gpu-utilization):
...
- **[Efficient Memory Management for Large Language Model Serving with PagedAttention](https://arxiv.org/abs/2309.06180)** — The vLLM paper introducing PagedAttention (Kwon et al., SOSP 2023)
- **[vLLM: Easy, Fast, and Cheap LLM Serving](https://docs.vllm.ai/)** — Official vLLM documentation with architecture details
- **[TensorRT-LLM Documentation](https://nvidia.github.io/TensorRT-LLM/)** — NVIDIA's official TensorRT-LLM docs covering compilation and deployment
- **[Automating Inference Optimizations with NVIDIA TensorRT LLM AutoDeploy](https://developer.nvidia.com/blog/automating-inference-optimizations-with-nvidia-tensorrt-llm-autodeploy)** — NVIDIA blog on AutoDeploy for simplified deployment
- **[TensorRT-LLM AutoDeploy Documentation](https://nvidia.github.io/TensorRT-LLM/torch/auto_deploy/auto-deploy.html)** — Official AutoDeploy docs with support matrix
- **[NGC Catalog: TensorRT-LLM Containers](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/tensorrt-llm/containers/release)** — Pre-built containers and engines
- **[In-Flight Batching in TensorRT-LLM](https://developer.nvidia.com/blog/nvidia-tensorrt-llm-supercharges-large-language-model-inference-on-nvidia-h100-gpus/)** — NVIDIA blog on TensorRT-LLM architecture
- **[Orca: A Distributed Serving System for Transformer-Based Generative Models](https://www.usenix.org/conference/osdi22/presentation/yu)** — Original continuous batching paper (Yu et al., OSDI 2022)
